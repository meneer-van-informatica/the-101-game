...
def _draw_dog(surface, center, t, blink_phase=False):
    """Teken een eenvoudige hond met wagging tail."""
    cx, cy = center
    # Basis kleuren
    fur = (198, 162, 110)
    ear = (168, 132, 90)
    ...
    # Tail tip
    tip = (
        int(tail_end[0] + 12 * math.cos(wag)),
        int(tail_end[1] + 12 * math.sin(wag)),
    )
    ...
    eye_dx = int(22*s)
    if blink_phase:
        # gesloten ogen: kleine lijntjes
        pygame.draw.line(surface, black, (head_center[0]-eye_dx-8, eye_y),
                         (head_center[0]-eye_dx+8, eye_y), 2)
        pygame.draw.line(surface, black, (head_center[0]+eye_dx-8, eye_y),
                         (head_center[0]+eye_dx+8, eye_y), 2)
    else:
...
class LevelStoryOne:
    """W0 • Distance Zero — startscherm met hond. Enter keert terug naar het menu."""
    wants_beat = True  # metronoom aan op dit startscherm
    ...
    def __init__(self, services):
        ...
        # optioneel: sfx-namen (als aanwezig in data/sfx)
        self._sfx_click = 'click'

    def handle_event(self, e):
        if e.type == pygame.KEYDOWN:
            if e.key in (pygame.K_RETURN, pygame.K_SPACE):
                # markeer W0 klaar en terug naar het menu
    ...
    def draw(self, screen):
        ...
        c = int(180 + 60 * pulse)
        hint = self.font_hint.render('Press Enter to start • Esc to menu', True, (c, c, c))
